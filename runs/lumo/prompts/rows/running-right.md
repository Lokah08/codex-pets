Create a single horizontal sprite strip for the Codex app digital pet `lumo` in the state `running-right`.
...
Output exactly 8 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: Lumo is a rounded hot-pink toy-like companion with a large dome head, simple body, stubby arms and legs, and two glossy lime-green eyes. Preserve the pink body, lime eyes, rounded silhouette, tiny limbs, and friendly quiet expression..
